...
- **Blood Sugar** (`/glucose`) — restricted to one designated owner only (not just private-per-user like BP/Hikes — the other adult can't see this menu item or reach the routes at all, 404 either way). Log: date, optional time, reading in mg/dL, a context tag (fasting / after meal / random, each with helper text), notes. The category badge depends on context — fasting uses a lower threshold set than after-meal/random, since their normal ranges genuinely differ (descriptive only, not medical advice). `/glucose/trends` charts readings over time (colored by context, hover a point to see its note — handy for "what did I eat"), plus averages overall/by-context/by-week.
